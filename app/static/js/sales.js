...
function renderCart() {
  ...
  if (!saleState.cart.length) {
    container.innerHTML = '<p class="empty">Añade productos al carrito.</p>';
  } else {
  ...
  const t = totals();
  t.discount = t.subtotal - t.total;
  $('#cartSubtotal').textContent = saleMoney(t.subtotal);
  $('#cartDiscount').textContent = saleMoney(t.discount);
