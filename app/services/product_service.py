    ...
    with db_cursor() as cursor:
        cursor.execute(
            ...
            SELECT p.id AS producto_id,
                   ...
                   CASE WHEN pp.id IS NOT NULL AND pr.id IS NOT NULL THEN 1 ELSE 0 END AS vendible,
                   COALESCE(MAX(stock.stock_total), 0) AS stock_total,
                   COALESCE(MAX(stock.stock_ubicaciones), 'Sin stock registrado') AS stock_ubicaciones
            FROM productos p
            JOIN categorias_producto c ON c.id = p.categoria_id
            LEFT JOIN producto_presentaciones pp
                   ON pp.producto_id = p.id AND pp.estado = 'ACTIVO'
            LEFT JOIN producto_precios pr
                   ON pr.producto_presentacion_id = pp.id AND pr.estado = 'ACTIVO'
            LEFT JOIN (
                SELECT i.cliente_id,
                       i.producto_id,
            ...
            WHERE p.cliente_id = %s
              ...
              AND (
                 ...
                 OR p.codigo_producto LIKE %s
                 OR COALESCE(p.codigo_barras, '') LIKE %s
              )
            GROUP BY p.id, pp.id, pr.id, c.nombre
            ORDER BY p.nombre ASC, COALESCE(pp.factor_unidad_base, 1) ASC
            LIMIT %s
            """,
    ...
    with db_cursor() as cursor:
        cursor.execute(
            ...
            WHERE i.cliente_id = %s
              AND (p.nombre LIKE %s OR COALESCE(p.descripcion, '') LIKE %s OR p.codigo_producto LIKE %s OR COALESCE(p.codigo_barras, '') LIKE %s OR u.nombre LIKE %s)
